...
class Ui_OptionsWindow(object):
    ...
    def openDataWindow(self):
        ...
        if self.checkBoxRijeka.isChecked():
            ...
            #dataset = do.load_dataset("data/Rijeka.csv", gradovi[0])
        if self.checkBoxMarcelji.isChecked():
            gradovi.append("Marcelji")
            #dataset = do.load_dataset("data/Marcelji.csv", gradovi[0])
        if self.checkBoxMatulji.isChecked():
            gradovi.append("Matulji")
            #dataset = do.load_dataset("data/Matulji.csv", gradovi[0])
        ...
        if self.danRadio.isChecked():
            gt.get_dan_data(self, dataset_1, gradovi[0])

        if self.mjesecRadio.isChecked():
            if numGradovi == 1:
                gt.get_mjesec_data(self, dataset_1, gradovi[0])
            elif numGradovi == 2:
                gt.compare_mjesec_data_two(self, dataset_1, dataset_2, gradovi[0], gradovi[1])
            else:
                gt.compare_mjesec_data_three(self, dataset_1, dataset_2, dataset_3, gradovi[0], gradovi[1], gradovi[2])

        if self.godinaRadio.isChecked():
            gt.get_godina_data(self, dataset_1)

        if self.rasponRadio.isChecked():
            gt.get_raspon_data(self, dataset_1)

        if self.usporedbaRadio.isChecked():
            gt.get_usporedba_data(self)
